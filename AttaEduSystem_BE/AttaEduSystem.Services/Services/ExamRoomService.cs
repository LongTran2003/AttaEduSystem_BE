    ...
    public class ExamRoomService : IExamRoomService
    {
        ...
        public async Task<ResponseDto> CreateRoomsBatch(List<CreateExamRoomDto> dtos, ClaimsPrincipal user)
        {
            if (dtos == null || dtos.Count == 0)
                return ErrorResponse.Build("Room list is empty", 400);
            ...
            {
                var result = await CreateRoom(dto, user);
                if (result.IsSuccess)
                {
                    created.Add(result.Result ?? new { });
                }
                else
                {
                    failed.Add(new
            ...
            var statusCode = created.Count > 0 ? 201 : 400;
            return SuccessResponse.Build(
                "Batch room creation processed",
                statusCode,
                new
                {
        ...
        public async Task<ResponseDto> UpdateRoomSchedule(Guid examRoomId, DateTime newStartTime, int? timeLimit, ClaimsPrincipal user)
        {
            ...
            if (!CanManageRoom(user, room, userId))
                return ErrorResponse.Build("You do not have permission to update this room", 403);

            var normalizedStart = newStartTime.Kind == DateTimeKind.Utc ? newStartTime.AddHours(7) : newStartTime;
            if (normalizedStart <= StaticOperationStatus.Timezone.Vietnam)
                return ErrorResponse.Build("Start time must be in the future", 400);

            room.StartTime = normalizedStart;
            if (timeLimit.HasValue && timeLimit.Value > 0)
                room.TimeLimit = timeLimit.Value;

            room.EndTime = room.StartTime.AddMinutes(room.TimeLimit);
            ...
            return SuccessResponse.Build("Room schedule updated successfully", 200, new
            {
                room.ExamRoomId,
                room.RoomCode,
                room.StartTime,
        ...
        public async Task<ResponseDto> GetSelectedExamByRoomId(Guid examRoomId, ClaimsPrincipal user)
        {
            ...
            return SuccessResponse.Build("Selected exam retrieved successfully", 200, paper);
        }
        ...
        public async Task<ResponseDto> GetRoomResults(Guid examRoomId, ClaimsPrincipal user)
        {
            ...
            if (!CanManageRoom(user, room, userId))
                return ErrorResponse.Build("You do not have permission to view room results", 403);

            var attemptIds = room.Participants
                .Where(p => p.ExamAttemptId.HasValue)
                .Select(p => p.ExamAttemptId!.Value)
                .Distinct()
                .ToList();

            var attempts = (await _unitOfWork.ExamAttempt.GetAllAsync(
                    a => attemptIds.Contains(a.ExamAttemptId),
                    includeProperties: "User"))
                .ToList();

            var results = room.Participants
                .Select(p =>
                {
                    var attempt = p.ExamAttemptId.HasValue
                        ? attempts.FirstOrDefault(a => a.ExamAttemptId == p.ExamAttemptId.Value)
                        : null;
        ...
        public async Task<ResponseDto> GetTeacherDashboard(DateTime? date, ClaimsPrincipal user)
        {
            ...
            var targetDate = (date ?? StaticOperationStatus.Timezone.Vietnam).Date;
            var rooms = await _unitOfWork.ExamRoom.GetByCreatorIdAsync(userId);
        ...
        public async Task<ResponseDto> JoinRoomById(Guid examRoomId, ClaimsPrincipal user)
        {
            ...
            return await JoinRoom(room.RoomCode, user);
        }
        ...
        public async Task<ResponseDto> GetPaperForTakingByRoomId(Guid examRoomId, ClaimsPrincipal user)
        {
            ...
            return await GetPaperForTaking(room.RoomCode, user);
        }
        ...
        private static bool CanManageRoom(ClaimsPrincipal user, ExamRoom room, string userId)
        {
            if (user.IsInRole(StaticUserRoles.Admin))
                return true;

            if (string.Equals(room.CreatedBy, userId, StringComparison.OrdinalIgnoreCase))
                return true;

            var fullName = user.FindFirstValue("FullName");
            return !string.IsNullOrWhiteSpace(fullName) &&
                   string.Equals(room.CreatedBy, fullName, StringComparison.OrdinalIgnoreCase);
        }
    }
}
